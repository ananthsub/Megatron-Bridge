...
class NemotronHFinetuneKwargs(TypedDict, total=False):
...
def nemotronh_4b_finetune_config(**user_kwargs: Unpack[NemotronHFinetuneKwargs]) -> ConfigContainer:
    ...
    - Full SFT: TP=1, PP=1, SP=False, LR=5e-6
    """
    ...
    # Build base config
    config = _nemotronh_finetune_common(hf_path="nvidia/Nemotron-H-4B-Base-8K", **user_kwargs)
...
def nemotronh_8b_finetune_config(**user_kwargs: Unpack[NemotronHFinetuneKwargs]) -> ConfigContainer:
    ...
    Default configuration: 8 nodes, 64 GPUs, Full SFT only
    - Full SFT: TP=2, PP=1, SP=True, LR=5e-6
    """
    # Auto-select LR if not specified
    finetune_lr = user_kwargs.get("finetune_lr")
    if finetune_lr is None:
    ...
    # Build base config
    config = _nemotronh_finetune_common(hf_path="nvidia/Nemotron-H-8B-Base-8K", **user_kwargs)
...
def nemotronh_47b_finetune_config(**user_kwargs: Unpack[NemotronHFinetuneKwargs]) -> ConfigContainer:
    ...
    Default configuration: 32 nodes, 256 GPUs, Full SFT only
    - Full SFT: TP=8, PP=1, SP=True, LR=5e-6
    ...
    # Use FP8 precision by default for 47B
    if "precision_config" not in user_kwargs:
        user_kwargs["precision_config"] = "nemotron_h_bf16_with_fp8_current_scaling_mixed"

    # Build base config
    config = _nemotronh_finetune_common(hf_path="nvidia/Nemotron-H-47B-Base-8K", **user_kwargs)
...
def nemotronh_56b_finetune_config(**user_kwargs: Unpack[NemotronHFinetuneKwargs]) -> ConfigContainer:
    ...
    # Use FP8 precision by default for 56B
    if "precision_config" not in user_kwargs:
        user_kwargs["precision_config"] = "nemotron_h_bf16_with_fp8_current_scaling_mixed"

    # Build base config
    config = _nemotronh_finetune_common(hf_path="nvidia/Nemotron-H-56B-Base-8K", **user_kwargs)
    config.model.tensor_model_parallel_size = 8
    config.model.pipeline_model_parallel_size = 1
    config.model.sequence_parallel = True
...
def _nemotronh_finetune_common(
    ...
    """
    Create a finetuning configuration for NemotronH models using AutoBridge.

    Args:
        hf_path (str): HuggingFace model path (e.g., "nvidia/Nemotron-H-4B-Base-8K").
    ...
    # Create model config using AutoBridge
    ...
    opt_config, scheduler = distributed_fused_adam_with_cosine_annealing(
        ...
        max_lr=finetune_lr if finetune_lr is not None else 5e-6,
        min_lr=min_lr,
        adam_beta1=0.9,
        adam_beta2=0.95,
        adam_eps=1e-5,
    ...
    # Enable default comm overlap for NemotronH
    cfg.comm_overlap = CommOverlapConfig(
        tp_comm_bootstrap_backend="nccl",
        tp_comm_overlap=True,
    )
