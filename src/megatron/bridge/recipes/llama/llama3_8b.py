...
def finetune_config(
    ...
    """
    Create a fine-tuning configuration for Llama3 8B model.
    ...
    The configuration is optimized for running on 1 node with 8 GPUs by default.
    ...
    Examples:
        Convert and fine-tune a HuggingFace model:
            ...
            >>> bridge = AutoBridge.import_ckpt("meta-llama/Meta-Llama-3-8B", "/path/to/checkpoint")
    ...
    # Model configuration - TP=1 for all cases (matches NeMo performance mode)
    ...
    # Communication overlap for all cases
    ...
    cfg = ConfigContainer(
        ...
        optimizer=opt_config,
        scheduler=scheduler,
        ddp=_create_ddp_config_with_performance_opts(use_distributed_optimizer, peft_config),
        ...
        tokenizer=create_tokenizer_config(model_id="meta-llama/Meta-Llama-3-8B"),
...
def _create_ddp_config_with_performance_opts(
    ...
    """
    Create DDP config with performance optimizations for Llama3 8B.
