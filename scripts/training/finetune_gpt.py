...
"""
Finetune Script for GPT-based Models
...
Recipe Arguments:
    ...
    (e.g., specific PEFT configurations at build time), use the
    model-specific examples in examples/recipes/<family>/ or create
    a custom script.
"""
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser = argparse.ArgumentParser(
        description="Generic finetune script for GPT-based models",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Recipe function name (e.g., llama32_1b_finetune_config, gemma3_1b_finetune_config)",
    )
    parser.add_argument(
        "--config-file",
        type=str,
...
def load_recipe(recipe_name: str) -> ConfigContainer:
    ...
    Args:
        recipe_name: Full recipe function name (e.g., 'llama32_1b_finetune_config')
    ...
    if not hasattr(recipes, recipe_name):
        raise AttributeError(
            ...
            f"Example recipe names: llama32_1b_finetune_config, gemma3_1b_finetune_config, qwen3_8b_finetune_config"
        )
