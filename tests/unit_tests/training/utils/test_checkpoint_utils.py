...
class TestIsIterationDirectory:
    ...
    def test_valid_iteration_directory(self):
        """Test that valid iteration directory names are recognized."""
    ...
    def test_malformed_iteration_directory(self):
        ...
        assert is_iteration_directory("iter_1") is False
        assert is_iteration_directory("iter_12") is False
        assert is_iteration_directory("iter_123456") is False  # 6 digits
...
class TestResolveCheckpointPath:
    ...
    def test_resolve_ignores_malformed_iter_directories(self, tmp_path):
        """Test that malformed iteration directories are ignored."""
        checkpoint_dir = tmp_path / "checkpoints"
        checkpoint_dir.mkdir()
        ...
        (checkpoint_dir / "iter_12").mkdir()  # Invalid: wrong number of digits
    ...
    def test_resolve_with_only_malformed_iter_directories_raises(self, tmp_path):
        ...
        # Create only invalid iteration directories
        (checkpoint_dir / "iter_invalid").mkdir()
        (checkpoint_dir / "iter_abc").mkdir()
        (checkpoint_dir / "iter_12").mkdir()
