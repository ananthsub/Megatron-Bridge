...
class TestModelConfig:
    ...
    def test_model_config_default_parameters(self):
        ...
        assert isinstance(config, NemotronHModel56BProvider)
    ...
    def test_model_config_custom_tensor_parallelism(self):
        ...
        config = model_config(tensor_parallelism=4)  # NemotronH 56B has 8 query groups, max TP is 8
    ...
    def test_model_config_all_custom_parameters(self):
        ...
        config = model_config(
            tensor_parallelism=4,  # NemotronH 56B has 8 query groups, max TP is 8
...
class TestPretrainConfig:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (8, 1, 1),  # Default tensor parallelism (max for 56B due to 8 query groups)
            (4, 1, 4),
            (2, 2, 2),
        ],
    )
