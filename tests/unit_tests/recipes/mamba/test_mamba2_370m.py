...
from megatron.bridge.models.mamba import MambaProvider370M
from megatron.bridge.recipes.mamba.mamba2_370m import model_config, pretrain_config
...
class TestModelConfig:
    ...
    def test_model_config_default_parameters(self):
        ...
        assert isinstance(config, MambaProvider370M)
    ...
    def test_model_config_all_custom_parameters(self):
        ...
        config = model_config(
            tensor_parallelism=1,  # Mamba 370M has only 1 attention head
            pipeline_parallelism=4,
            pipeline_parallelism_dtype=torch.bfloat16,
            virtual_pipeline_parallelism=8,
            context_parallelism=16,
            sequence_parallelism=False,  # Cannot use sequence parallelism with tensor_parallelism=1
        )
...
class TestPretrainConfig:
    ...
    def test_pretrain_config_default_comm_overlap(self):
        ...
        # Default setup should have comm overlap as None
