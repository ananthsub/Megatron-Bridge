...
from megatron.bridge.recipes.mamba.mamba2_8b import model_config, pretrain_config
...
class TestModelConfig:
    ...
    def test_model_config_default_parameters(self):
        ...
        assert isinstance(config, NVIDIAMambaProvider8B)
    ...
    def test_model_config_custom_tensor_parallelism(self):
        ...
        config = model_config(tensor_parallelism=16)  # Mamba 8B has 32 attention heads
    ...
    def test_model_config_sequence_parallelism_enabled(self):
        ...
        config = model_config(sequence_parallelism=True, tensor_parallelism=16)  # Mamba 8B has 32 attention heads
    ...
    def test_model_config_all_custom_parameters(self):
        ...
        config = model_config(
            tensor_parallelism=16,  # Mamba 8B has 32 attention heads
...
class TestPretrainConfig:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (2, 1, 4),
            (4, 2, 2),
            (8, 2, 2),  # Default tensor parallelism
            (16, 2, 1),  # Higher tensor parallelism for 8B model
            (32, 1, 1),  # Maximum tensor parallelism for 8B model
        ],
    )
